		...
		<script type="text/javascript">
			$(function(){
				...
				$("#save").click(function(){//绑定点击事件
					if($("#standardForm").form("validate"))//校验，如果表单中都是有效内容就通过
					$("#standardForm").submit();
				})
			    // 退出后,清除表单中所有的内容
			...
			var toolbar = [ {
				...
				iconCls : 'icon-add',
				handler : function(){
					$("#standardWindow").window('open');//对象.组件.方法
				}
			}, {
				id : 'button-edit',
				text : '修改',
				...
				handler : function(){
					...
					$("#standardWindow").form("load",rows[0]);//加载数据
					$("#standardWindow").window("open");//弹出表单
			...
			<shiro:hasRole name="base">
			{
				...
				iconCls : 'icon-cancel',
				handler : function(){
					$("#standardWindow").form("clear");//清除表单的内容
				}
			},
			</shiro:hasRole>
			{
				...
				iconCls : 'icon-save',
				handler : function(){
					$("#standardWindow").form("reset");//还原表单的内容
				}
			}];
			// 定义列
			var columns = [ [ {
	...
	<body class="easyui-layout" style="visibility:hidden;">
		...
		<div class="easyui-window" title="对收派标准进行添加或者修改" id="standardWindow" collapsible="false" minimizable="false" maximizable="false" modal="true" closed="true" style="width:600px;top:50px;left:200px">
			...
			<div region="center" style="overflow:auto;padding:5px;" border="false">
				...
				<form id="standardForm" method="post" action="../../standard_save.action">
